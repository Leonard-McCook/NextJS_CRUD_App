...
export default function AddUserForm({ formData, setFormData }) {
    ...
    if(addMutation.isError) return <Error message={add.Mutation.error.message}></Error>
  ...
  return (
    <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
       ...
       <div className="input-type">
          <input type="date" onChange={setFormData} name="date" className="border px-5 py-4 focus:outline-none rounded-md" placeholder="Salary" />
       </div>
       ...
       <button className="flex justify-center text-md w-2/6 bg-green-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-green-500 hover:text-green-500">Add <span className="px-1"><BiPlus size={22}></BiPlus></span></button>
